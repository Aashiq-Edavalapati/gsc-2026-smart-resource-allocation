...
import { addContactDetails, getPendingOrgs, getStats, getUsers, getOrganizations } from '../controllers/admin.controller.js';

// Note: You need a middleware here to ensure req.user.role === 'PLATFORM_ADMIN'
const requirePlatformAdmin = (req, res, next) => {
  if (req.user.role !== 'PLATFORM_ADMIN') return res.status(403).json({ success: false, error: 'Unauthorized' });
  next();
};
